...
class MCPSchemaMonitor {
  ...
  private expectedTables = ['users', 'events', 'event_participants', 'media', 'messages'];
  ...
  async syncLiveSchema(): Promise<void> {
    ...
    try {
      ...
      const typesPath = path.join(process.cwd(), 'app/reference/supabase.types.ts');
      ...
      console.log('🎯 Schema sync complete - Cursor now has live database awareness');
  ...
  async validateSchemaIntegrity(): Promise<void> {
    ...
    const missingTables = this.expectedTables.filter(table => 
    ...
    if (!typesContent.includes('Functions:') || !typesContent.includes('is_event_host')) {
  ...
  async detectSchemaDrift(): Promise<void> {
    ...
    const schemaHash = Buffer.from(currentSchema).toString('base64').slice(0, 16);
...
if (require.main === module) {
  ...
  if (watchMode) {
    ...
    monitor.validateSchemaIntegrity()
      .then(() => {
