...
export const getEventParticipants = async (eventId: string): Promise<ServiceResponseArray<EventParticipantWithUser>> => {
  try {
    ...
    return {
      data: null,
      error: error instanceof Error ? error : new Error('Failed to get event participants')
    }
  }
}

export const getUserByPhone = async (phone: string): Promise<ServiceResponse<User>> => {
  try {
    const result = await supabase
      .from('users')
      .select('*')
    ...
    return {
      data: null,
      error: error instanceof Error ? error : new Error('Failed to get user by phone')
    }
  }
}

export const createUser = async (userData: UserInsert): Promise<ServiceResponse<User>> => {
  try {
    const result = await supabase
      .from('users')
      .insert(userData)
...
export const updateParticipant = async (id: string, updates: EventParticipantUpdate) => {
  try {
...
export const importParticipants = async (eventId: string, participants: Array<{
  name: string
  phone: string
  email?: string
  role?: 'host' | 'guest'
  ...
  try {
    ...
    const participantInserts: EventParticipantInsert[] = userResults.map((user, index) => ({
      event_id: eventId,
      user_id: user.id,
...
export const getParticipantsByRole = async (eventId: string, role: 'host' | 'guest') => {
  try {
    return await supabase
      .from('event_participants')
      .select(`
...
export const getEventGuests = getEventParticipants
export const getGuestByPhone = getUserByPhone  
export const createGuest = createUser
export const updateGuest = updateParticipant
export const deleteGuest = removeParticipant
export const importGuests = importParticipants
export const updateGuestRSVP = updateParticipantRSVP 

// Additional legacy functions for backward compatibility
export const inviteGuest = async (eventId: string, guestData: {
  name: string
  phone: string
  email?: string
}) => {
...
export const getGuestStats = async (eventId: string) => {
  try {
    ...
    const guests = participants.filter((p: EventParticipant) => p.role === 'guest')
    const stats = {
      total: guests.length,
      attending: guests.filter((g: EventParticipant) => g.rsvp_status === 'attending').length,
      declined: guests.filter((g: EventParticipant) => g.rsvp_status === 'declined').length,
      maybe: guests.filter((g: EventParticipant) => g.rsvp_status === 'maybe').length,
      pending: guests.filter((g: EventParticipant) => g.rsvp_status === 'pending').length
    }
