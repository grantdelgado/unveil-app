...
export const validateFileType = async (file: File): Promise<{ 
  isValid: boolean; 
  ...
  if (file.size > MAX_FILE_SIZE) {
    return { isValid: false, mediaType: null, error: 'File is too large (max 50MB)' }
  }
  
  // 2. Check for dangerous file extensions
  const hasDangerousExtension = DANGEROUS_EXTENSIONS.some(ext => fileName.endsWith(ext))
  if (hasDangerousExtension) {
    return { isValid: false, mediaType: null, error: 'File type not allowed for security reasons' }
  }
  ...
  if (ALLOWED_IMAGE_TYPES.includes(fileType)) {
    ...
    if (file.size > MAX_IMAGE_SIZE) {
      return { isValid: false, mediaType: null, error: 'Image is too large (max 25MB)' }
    }
    ...
    if (file.size > MAX_VIDEO_SIZE) {
      return { isValid: false, mediaType: null, error: 'Video is too large (max 50MB)' }
    }
  }
  ...
  if (!hasValidExtension) {
    return { isValid: false, mediaType: null, error: 'File extension not allowed' }
  }
  
  // 6. Ensure MIME type and extension match
  if (mediaType === 'image' && !ALLOWED_IMAGE_EXTENSIONS.some(ext => fileName.endsWith(ext))) {
    return { isValid: false, mediaType: null, error: 'File extension does not match MIME type' }
  }
  
  if (mediaType === 'video' && !ALLOWED_VIDEO_EXTENSIONS.some(ext => fileName.endsWith(ext))) {
    return { isValid: false, mediaType: null, error: 'File extension does not match MIME type' }
  }
  ...
  try {
    ...
    if (!isValidSignature) {
      return { isValid: false, mediaType: null, error: 'File content does not match declared type' }
    }
  } catch (error) {
...
export const validateMediaFile = async (file: File): Promise<{ isValid: boolean; error?: string; mediaType?: MediaType }> => {
  // Check file size
  ...
  if (!typeValidation.isValid) {
    return { 
      isValid: false, 
      error: typeValidation.error || 'Invalid file type. Please upload an image (JPG, PNG, WebP, GIF) or video (MP4, WebM, MOV, AVI).' 
    }
  }
