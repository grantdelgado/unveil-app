...
type GetUserEventsReturn = Database['public']['Functions']['get_user_events']['Returns'][0]
...
export function useUserEventsSorted(): UseUserEventsSortedReturn {
  ...
  const fetchAndSortEvents = useCallback(async () => {
    try {
      ...
      const sortedEvents = (userEvents || []).sort((a: GetUserEventsReturn, b: GetUserEventsReturn) => {
