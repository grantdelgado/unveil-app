...
function checkRateLimit(clientId: string, route: string): { allowed: boolean; remaining: number; resetTime: number } {
  const now = Date.now()
  ...
  if (!config) {
    ...
    const matchingRoute = Object.keys(RATE_LIMITS).find(key => route.startsWith(key))
    config = matchingRoute ? RATE_LIMITS[matchingRoute as keyof typeof RATE_LIMITS] : RATE_LIMITS['/api']
  }
