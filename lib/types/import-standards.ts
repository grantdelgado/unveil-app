...
export const STANDARD_IMPORT_PATTERNS = {
  ...
  REACT_HOOKS: "import { useState, useEffect, useCallback, useMemo } from 'react'",
  ...
  // Internal types
  APP_TYPES: "import type { Event, EventParticipant, Message, Media } from '@/lib/types'",
  ERROR_TYPES: "import type { AuthError, DatabaseError, MediaError } from '@/lib/types'",
  HOOK_TYPES: "import type { AuthHookResult, EventDetailsHookResult } from '@/lib/types'",
  
  // Services (prefer specific imports)
  AUTH_SERVICE: "import { sendOTP, verifyOTP, signOut } from '@/services/auth'",
  EVENTS_SERVICE: "import { createEvent, getEventDetails, updateEvent } from '@/services/events'",
  MEDIA_SERVICE: "import { uploadMedia, getEventMedia, deleteMedia } from '@/services/media'",
  
  // UI Components (prefer specific imports)
  UI_COMPONENTS: "import { Button, Input, LoadingSpinner } from '@/components/ui'",
  
  // Utilities
  LOGGER: "import { logAuth, logError, logDev } from '@/lib/logger'",
  VALIDATION: "import { validatePhoneNumber, validateEmail } from '@/lib/validations'",
  ERROR_HANDLING: "import { handleDatabaseError, withErrorHandling } from '@/lib/error-handling'"
} as const
...
export const IMPORT_VALIDATION_RULES: ImportValidationRule[] = [
  {
    ...
    message: 'Prefer specific imports over barrel exports for better tree-shaking: import { specificFunction } from "@/services/specificService"',
    severity: 'warning'
  },
  {
  ...
  },
  {
    ...
    message: 'Consider separating type imports: import type { TypeName } from "module"',
    severity: 'info'
  },
  {
    name: 'consistent-react-imports',
    pattern: /import\s+React\s*,\s*\{.*\}\s+from\s+['"]react['"]/,
    message: 'Consider importing React separately if not using JSX: import { useState } from "react"',
    severity: 'info'
  }
]
...
export const ImportUtils = {
   ...
   */
  validateImport: (importStatement: string): Array<{
    rule: string
    message: string
    severity: 'error' | 'warning' | 'info'
  }> => {
  ...
  toAbsoluteImport: (importStatement: string, _currentFilePath: string): string => {
